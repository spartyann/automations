...
function callApi($params)
{
    ...
    $url = 'https://lavoixduzen.fr?' . http_build_query($params);
    ...
    try {
        ...
        if($http_code == intval(200)){
            ...
            if ($responseJson == null) throw new \Exception("ERROR deconding Response API\n\n" . $response);
...
function saveDebug($data){
    ...
    $file = $dir . '/' . str_replace(':', '-', date('c'))  . '.json';
...
function formatCourseName($name){
    ...
    $name = trim($name);
    for ($i = 0; $i< 5; $i++) $name = str_replace('  ', '', $name);
    return $name;
}
